...
`length`表示后续数据的字节数。控制命令只发送；查询命令等待下位机准备数据后，再单独读取不带帧头的原始数据。机器人左右轮命令会作为一个命令整体入队，任意目标电机离线时整批拒绝；停止命令拥有最高优先级并取消目标电机尚未执行的运动命令。机器人转向使用板载Z轴累计角度反馈，不迁移参考平台内部的PID控制。
